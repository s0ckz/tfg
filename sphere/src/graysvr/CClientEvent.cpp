...
void CClient::Event_ChatButton(const NCHAR *pszName)
{
	...
	// Dirty fix for classic clients with Injection using chat button to exploit movement packets. We set
	// 'active trigger' to this, so we check it back on the packet to limit the amount of steps to do.
	GetChar()->SetTriggerActive("UserChatButton");

	ASSERT(m_pAccount);
	if ( m_pAccount->m_sChatName.IsEmpty() )
	{
		if ( m_NetState->isClientVersion(MINCLIVER_SA) || m_NetState->isClientKR() || m_NetState->isClientEnhanced() )
		{
			// Newest clients don't understand CHATMSG_GetChatName packet, so the name must be generated by the server.
			// By default it will set chat name = char name.
			CChat::GenerateChatName(m_pAccount->m_sChatName, this);
			if ( m_pAccount->m_sChatName.IsEmpty() )
			{
		...
		{
			// Check if the client sent any chat name
			if ( (pszName == NULL) || (pszName[0] == 0) )
			{
				// No data was sent, so ask for a permanent chat name (account based)
				addChatSystemMessage(CHATMSG_GetChatName);
				return;
			}

			// Make the chat name non-unicode and store it with the account stuff
			TCHAR szChatName[MAX_NAME_SIZE * 2 + 2];
			CvtNUNICODEToSystem(szChatName, sizeof(szChatName), pszName, 128);

			if ( !CChat::IsValidName(szChatName, true) || g_Accounts.Account_FindChat(szChatName) )		// check if the name is illegal or duplicated
			{
				addChatSystemMessage(CHATMSG_Error);
				addChatSystemMessage(CHATMSG_GetChatName);
				return;
